...
Exactplot = scipy.interpolate.griddata(
    (Xtest[:, 0], Xtest[:, 1]), Exact, (X, Y), method='linear')
Medplot = scipy.interpolate.griddata(
    (Xtest[:, 0], Xtest[:, 1]), Medium, (X, Y), method='linear')
Lowplot = scipy.interpolate.griddata(
    (Xtest[:, 0], Xtest[:, 1]), Low, (X, Y), method='linear')
...
print("N1 = %d, N2 = %d, N3 = %d, error = %e, GP error = %e" %
      (N1, N2, N3, error, gpe))


if plot == 1:
    Predplot = ml.griddata(Xtest[:, 0], Xtest[:, 1],
                           mu3.flatten(), X, Y, interp='linear')
    Varplot = ml.griddata(Xtest[:, 0], Xtest[:, 1],
                          v3.flatten(), X, Y, interp='linear')
    GPplot = ml.griddata(Xtest[:, 0], Xtest[:, 1],
                         mu4.flatten(), X, Y, interp='linear')
